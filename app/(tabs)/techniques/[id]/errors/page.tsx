  ...
  useEffect(() => {
    const loadTechnique = async () => {
      try {
        ...
        const errorHints = data.hints.filter(h => h.type === 'error');
        if (errorHints.length > 0) {
          setOpenAccordions([errorHints[0]?.id, errorHints[1]?.id].filter(Boolean));
        }
      } catch (err) {
              ...
              return (
                <div key={hint.id} className="">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between p-4 hover:bg-primary-200 transition rounded-2xl cursor-pointer"
                  >
                    <span className="font-semibold text-dark">{hint.title}</span>
                    <MdExpandMore
                      className={`w-6 h-6 text-dark-100 transition-transform ${isOpen ? 'rotate-180' : ''
                        }`}
                    />
                  </button>
